...
class AppFormatters {
  ...
  static String maskPhoneNumber(String phone) {
    ...
    final masked =
        '${'*' * (cleaned.length - 4)}${cleaned.substring(cleaned.length - 4)}';
    return '+223 $masked';
  }

  /// Nettoyer le nom d'une matière en supprimant les caractères spéciaux
  static String cleanSubjectName(String subjectName) {
    if (subjectName.isEmpty) return subjectName;

    // Supprimer les contenus entre crochets [ex: 10e] et parenthèses
    final cleaned = subjectName
        .replaceAll(RegExp(r'\[[^\]]*\]'), '') // Supprimer [contenu]
        .replaceAll(RegExp(r'\([^)]*\)'), '') // Supprimer (contenu)
        .replaceAll(RegExp(r'\{[^}]*\}'), '') // Supprimer {contenu}
        .replaceAll(
          RegExp(r'[\{\[\(\}\]\)]'),
          '',
        ) // Supprimer les caractères restants
