...
class ApiService {
  ...
  Future<void> _onError(
    ...
    final bool isTokenExpired = status == 401 &&
        (data is Map &&
            ((data['code']?.toString() == 'TOKEN_EXPIRED') ||
                (data['error_code']?.toString() == 'REFRESH_INVALID')));

    final requestOptions = error.requestOptions;
    final alreadyRetried = requestOptions.extra['retried'] == true;

    if (isTokenExpired && !isRefreshEndpoint && !alreadyRetried && _authService != null && !_refreshing) {
      _refreshing = true;
      try {
